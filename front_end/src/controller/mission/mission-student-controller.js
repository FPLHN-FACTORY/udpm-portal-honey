window.MissionStudentController = function ($http, $scope) {
    $scope.mission = [];
    $scope.currentPage = 1;
    $scope.pageSize = 5;
    $scope.size = 0;
    $http
      .get("http://localhost:2508/api/admin/mission/student")
      .then(function (response) {
        $scope.mission = response.data.data;
        $scope.size = $scope.mission.length
      });
    ...
    $scope.getMission  = function(index){
      let pro = $scope.mission[index];
      $http
      .put(
        `${"http://localhost:2508/api/admin/mission/add-mission-detail"}/${pro.id}`,
        $scope.form_mission
      )
      .then(function (response) {
        $scope.mission.splice(index, 1);
