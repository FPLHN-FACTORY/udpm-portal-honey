...
public interface TeacherUseGiftRequestRepository extends HistoryRepository {
    ...
    @Query(value = """
            ...
            h.student_id, g.name as nameGift, h.name_gift as lop, h.created_date, h.status as status
             from history h
             ...
             and (:#{#request.lop} is null or h.name_gift = :#{#request.lop})
    ...
    @Query("""
    select h.nameGift from History h
