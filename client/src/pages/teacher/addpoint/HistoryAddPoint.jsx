...
export default function HistoryAddPoint() {
  ...
  const data = useAppSelector(GetHistory).map((data) => {
    return {
      ...
      createdDate: dayjs(data.createdDate).format("DD-MM-YYYY"),
    ...
    <div className="add-point">
      <Card className="mb-2 py-1">
        <Form onFinish={onFinishSearch}>
          <Space size={"large"}>
            <Form.Item name="textSearch" className="search-input">
              <Input
                style={{ width: "300px" }}
                name="textSearch"
                size="small"
                placeholder="Nhập mã hoặc tên sinh viên cần tìm"
